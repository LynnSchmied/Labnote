...
exp.mean <- 0.2  # mean value for exponential distributed C data
decim.num <- 10
obs <- 35  # number of observations
...
decim.hill <- model.hill %>%
  subset(select = c(exp_c, exp_y)) %>%
  mutate(group = ntile(exp_c, decim.num)) %>%
  group_by(group) %>%
  summarise_all(list(mean, sd), na.rm = TRUE) %>%
  rename(y_mean = exp_y_fn1,
         y_sd = exp_y_fn2,
         c_mean = exp_c_fn1,
         c_sd = exp_c_fn2)

ggplot() +
  geom_point(data = model.hill, aes(x = exp_c, y = exp_y),
             colour = 'blue',
             fill = 'blue',
             alpha = .5) +
  geom_point(data = decim.hill, aes(x = c_mean, y = y_mean),
             colour = 'red',
             fill = 'red',
             alpha = .5,
             size = 2)
...
DRP <- ggplot(model.hill) +
  ...
  geom_point(aes(x = exp_c, y = exp_y),
             ...
             alpha = .6,
             size = 2.5) +
  geom_point(data = decim.hill,
             aes(x = c_mean, y = y_mean),
             colour = 'green',
             alpha = .6,
             size = 3) +
...
HP <- ggplot(model.hill) + 
  ...
  geom_point(aes(x = log10(exp_c), y = log10(exp_y/(1-exp_y))),
             alpha = .6,
             colour = 'blue') +
  geom_point(data = decim.hill, aes(x = log10(c_mean), y = log10(y_mean/(1-y_mean))),
             alpha = .6,
             colour = 'green') +
